...
While the callback is dumping data into the buffer, the Worker Thread is waking up constantly to check the buffer.
    ...
    Read: It pulls 1024 samples out of the Input Ring Buffer.
    ...
    Accumulate: It takes this clean chunk and adds it to a growing list called burst_buffer.

    Wait: It repeats this until burst_buffer holds exactly 5 seconds of audio.
...
This is the core AI. The voice.onnx model takes two inputs:
    ...
    The Content (from Hubert).

    The New Pitch (from Crepe).
